...
from models.decoders.upernet import UperNet_swin
...
def main(config, local_rank):
    ...
    model = UperNet_swin(encoder=encoder, num_classes=18)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            # logger.info(f'Traing accumulation above 1')
            ...
            # logger.info(f'Train accumulation below 2')
            loss = criterion(outputs, targets.float())
            optimizer.zero_grad()
            if config.AMP_OPT_LEVEL != "O0":
                with amp.scale_loss(loss, optimizer) as scaled_loss:
...
if __name__ == '__main__':
    ...
    joblib.dump(config, f'{config.TAG}.config.sav')

    main(config, local_rank=local_rank)
